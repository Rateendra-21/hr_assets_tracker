...
const convertEventsToCSV = (groupedEvents) => {
  ...
  groupedEvents.forEach((group) => {
    const cycleNum = `Allocation Cycle ${group.cycleNumber}`;
...
// ---------------------- LifeCycle Component ----------------------
const LifeCycle = ({ data, onClose }) => {
  const [groupedEvents, setGroupedEvents] = useState([]);
  const [selectedUser, setSelectedUser] = useState("All");

  useEffect(() => {
    if (data?.events) {
      const grouped = groupByAllocationCycles(data.events);
  ...
  // Filtered cycles based on selected user
  ...
  const handleDownloadReport = () => {
    if (!filteredCycles.length) return alert("No events to download");
    const csv = convertEventsToCSV(filteredCycles);
    const filename = `asset_${asset.asset_name.replace(
      /\s+/g,
      "_"
    )}_report.csv`;
              ...
              filteredCycles.map((group) => (
                <div key={group.cycleNumber} className="position-relative mb-4">
                  ...
                  <div className="position-relative ps-3">
                    ...
                    <hr
                      style={{ borderTop: "2px dashed #ccc", margin: "10px 0" }}
                    />
                  </div>
                </div>
              ))
